...
{
	echo "<script>window.location='panchayatrajaccount.php';</script>";
}
$err='';
if(isset($_POST['submit']))
{
	$sql = "SELECT * FROM panchayatraj WHERE loginid='$_POST[loginid]' AND password='$_POST[password]' AND status='Active'";
	$qsql = mysqli_query($con,$sql);
	if(mysqli_num_rows($qsql) == 1)
	{
		$rslogin = mysqli_fetch_array($qsql);
		$_SESSION['panchayatrajid']= $rslogin['panchayatrajid'] ;
		echo "<script>window.location='panchayatrajaccount.php';</script>";
	}
	else
	{
...
?></div>
    ...
    <div class="card">
        <h1 class="title"><span>Grassroots Leadership in Action</span>Login <span class="msg">Hello, PanchayatRaj!</span></h1>
        <div class="col-md-12">
...
{
	 if(document.frmpanlogin.loginid.value == "")
	{
		alert("Login ID should not be empty..");
		document.frmpanlogin.loginid.focus();
		return false;
	}
	else if(!document.frmpanlogin.loginid.value.match(alphanumericExp))
	{
		alert("Login ID not valid..");
		document.frmpanlogin.loginid.focus();
		return false;
	}
	else if(document.frmpanlogin.password.value == "")
	{
		alert("Password should not be empty..");
		document.frmpanlogin.password.focus();
		return false;
	}
	else if(document.frmpanlogin.password.value.length < 8)
	{
